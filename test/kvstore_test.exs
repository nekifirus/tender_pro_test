defmodule KVstore.Test do
  ...
  test "create/1 creates new record in table" do
    new_key = "new_key"
    new_value = "new_value"
    assert {:ok, {:new_key, new_value}} = KVstore.create({new_key, new_value})

    assert :ets.lookup(@table_name, :new_key) == [new_key: new_value]
  ...
  test "update/1 existing record or error" do
    ...
    assert :ets.lookup(@table_name, :existed_key) == [existed_key: "new_value"]
